...
namespace Influx::Graphics
{
	VulkanAPI::VulkanAPI()
	{
		CreateInstance(m_vkInstance, "None");

#if _DEBUG
		m_vkDebugMessenger = VulkanAPI::SetupDebugMessenger(m_vkInstance);
#endif

		mp_mainDevice = new VulkanDevice(PickFirstSuitablePhysicalDevice(VulkanAPI::GetPhysicalDevices(m_vkInstance)));
		mp_mainDevice->CreateLogicalDevice({}, {}, true);
	}
	...
	RHICommandQueue* VulkanAPI::CreateCommandQueue(const ERHICommandQueueType type) const
	{
		...
		vulkanCmdQueue->m_vkCommandQueue = GetDevice().RequestDeviceQueue(Conversion::ToVulkan(type));
		vulkanCmdQueue->m_vkCommandPool = VulkanAPI::CreateCommandPool(GetDevice().VkLogicalDevice, GetDevice().QueueFamilyIndices.Graphics);
	...
	RHISwapChain* VulkanAPI::CreateSwapChain(HINSTANCE i, HWND windowHandle, RHICommandQueue* commandQueue) const
	{
		...
		{
			vulkanSwpChn->Width = static_cast<uint32_t>(rect.right - rect.left);
			vulkanSwpChn->Height = static_cast<uint32_t>(rect.bottom - rect.top);
		}
		...
		vulkanSwpChn->m_vkSurface = m_vkInstance.createWin32SurfaceKHR(createInfo);

		// Create the VkSwapChain
		const SwapChainSupportDetails& swpChnSupportDetails = VulkanAPI::QuerySwapChainSupport(mp_mainDevice->VkPhysicalDevice, vulkanSwpChn->m_vkSurface);
		...
		bool gfxQueueSupportsPresent = mp_mainDevice->VkPhysicalDevice.getSurfaceSupportKHR((uint32_t)GetDevice().GetQueueFamilyIndex(vk::QueueFlagBits::eGraphics), vulkanSwpChn->m_vkSurface);
		
		vk::SwapchainCreateInfoKHR swpChnCreateInfo{};
		swpChnCreateInfo.surface = vulkanSwpChn->m_vkSurface;
		...
		vulkanSwpChn->m_vkSwapChain = GetDevice().VkLogicalDevice.createSwapchainKHR(swpChnCreateInfo, nullptr);
		
		// Get Vulkan swapchain images:
		vulkanSwpChn->m_vkSwapChainImages = GetDevice().VkLogicalDevice.getSwapchainImagesKHR(vulkanSwpChn->m_vkSwapChain);
		vulkanSwpChn->m_vkExtent = chosenSwapExtent;
		vulkanSwpChn->m_vkImageFormat = chosenFormat.format;

		// Create Image views:
		vulkanSwpChn->m_vkImageViews = std::vector<vk::ImageView>{};
		for (int i = 0; i < vulkanSwpChn->m_vkSwapChainImages.size(); ++i)
			vulkanSwpChn->m_vkImageViews.push_back(VulkanAPI::CreateImageView(GetDevice().VkLogicalDevice, vulkanSwpChn->m_vkSwapChainImages[i], chosenFormat.format));
	...
	RHIShader* VulkanAPI::CreateRHIShader(const std::vector<uint8_t>& fromCompiledData, ERHIShaderType shaderType, ERHIShaderModel shaderModel) const
	{
		return new VulkanShader(fromCompiledData, shaderType, shaderModel);
	}
	...
	VulkanAPI::~VulkanAPI()
	{
		...
		delete mp_mainDevice;
		
		vkDestroyInstance(m_vkInstance, nullptr);
	}

	bool VulkanAPI::CheckVkResult(const vk::Result& result)
	{
		...
		switch (result)
		{
		case vk::Result::eErrorOutOfDeviceMemory: break;
		default:
			assert(false);
		}
...
#pragma region VulkanStatics
	void VulkanAPI::CreateInstance(vk::Instance& outResult, bool validation, const std::string& appName)
	{
		...
		CheckVkResult(vk::enumerateInstanceExtensionProperties(nullptr, &extensionCount, nullptr));
		std::vector<vk::ExtensionProperties> extensions(extensionCount);
		CheckVkResult(vk::enumerateInstanceExtensionProperties(nullptr, &extensionCount, extensions.data()));
	...
	{
		uint32_t layerCount;
		CheckVkResult(vk::enumerateInstanceLayerProperties(&layerCount, nullptr));
		std::vector<vk::LayerProperties> layers(layerCount);
		CheckVkResult(vk::enumerateInstanceLayerProperties(&layerCount, layers.data()));
	...
	vk::DebugUtilsMessengerEXT VulkanAPI::SetupDebugMessenger(const vk::Instance& instance)
	{
...
#pragma region VulkanCommandList
	void VulkanCommandList::RecordRenderPass(RHIRenderPass* renderPass, const RHIRenderPassBeginInfo& beginInfo, Function<void(RHICommandList* cmdList)> func)
	{
		...
		m_vkCommandBuffer.beginRenderPass(info, vk::SubpassContents::eInline);
		{
			func(this);
		}
	...
	void VulkanCommandList::ClearTextureAsRTV(RHITexture* texture, bool forceTransition)
	{
		...
		m_vkCommandBuffer.clearColorImage(vulkanTexture->GetVulkanImage(), Conversion::ToVulkan(vulkanTexture->GetRHIResource()->GetCurrentState()), value, {});
	}
...
#pragma region VulkanCommandQueue
	RHICommandList* VulkanCommandQueue::SetupNewCommandList(GraphicsAPI* api)
	{
		...
		newVulkanCommandList->m_vkCommandBuffer = 
			vulkanApi->GetDevice().VkLogicalDevice.allocateCommandBuffers(vk::CommandBufferAllocateInfo(m_vkCommandPool, vk::CommandBufferLevel::ePrimary, 1)).front();

		// Begin the new commandlist:
		vk::CommandBufferBeginInfo beginInfo{};
		beginInfo.sType = vk::StructureType::eCommandBufferBeginInfo;
		beginInfo.flags = vk::CommandBufferUsageFlagBits::eSimultaneousUse;
		newVulkanCommandList->m_vkCommandBuffer.begin(beginInfo);
...
#pragma region VulkanSwapChain
	void VulkanSwapChain::Present(RHICommandQueue* commandQueue, bool VSync)
	{
		...
		VulkanAPI::CheckVkResult(vulkanCommandQueue->GetVulkanQueue().presentKHR(&presentInfo));
	}
	...
	{
		vkDestroySurfaceKHR(VulkanAPI::Get().GetInstance(), m_vkSurface, nullptr);
	}
#pragma endregion
}
