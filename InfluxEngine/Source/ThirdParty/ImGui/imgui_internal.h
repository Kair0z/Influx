...
template<typename _T> static inline _T ImMin(_T lhs, _T rhs)                        { return lhs < rhs ? lhs : rhs; }
template<typename _T> static inline _T ImMax(_T lhs, _T rhs)                        { return lhs >= rhs ? lhs : rhs; }
template<typename _T> static inline _T ImClamp(_T v, _T mn, _T mx)                   { return (v < mn) ? mn : (v > mx) ? mx : v; }
template<typename _T> static inline _T ImLerp(_T a, _T b, float t)                  { return (_T)(a + (b - a) * t); }
template<typename _T> static inline void ImSwap(_T& a, _T& b)                      { _T tmp = a; a = b; b = tmp; }
template<typename _T> static inline _T ImAddClampOverflow(_T a, _T b, _T mn, _T mx)   { if (b < 0 && (a < mn - b)) return mn; if (b > 0 && (a > mx - b)) return mx; return a + b; }
template<typename _T> static inline _T ImSubClampOverflow(_T a, _T b, _T mn, _T mx)   { if (b > 0 && (a < mn + b)) return mn; if (b < 0 && (a > mx + b)) return mx; return a - b; }
// - Misc maths helpers
...
struct ImSpan
{
    ...
    inline ImSpan(_T* data, int size)                { Data = data; DataEnd = data + size; }
    inline ImSpan(_T* data, _T* data_end)             { Data = data; DataEnd = data_end; }

    inline void         set(_T* data, int size)      { Data = data; DataEnd = data + size; }
    inline void         set(_T* data, _T* data_end)   { Data = data; DataEnd = data_end; }
    inline int          size() const                { return (int)(ptrdiff_t)(DataEnd - Data); }
    inline int          size_in_bytes() const       { return (int)(ptrdiff_t)(DataEnd - Data) * (int)sizeof(_T); }
    inline _T&           operator[](int i)           { _T* p = Data + i; IM_ASSERT(p >= Data && p < DataEnd); return *p; }
    inline const _T&     operator[](int i) const     { const _T* p = Data + i; IM_ASSERT(p >= Data && p < DataEnd); return *p; }

    inline _T*           begin()                     { return Data; }
    inline const _T*     begin() const               { return Data; }
    inline _T*           end()                       { return DataEnd; }
    inline const _T*     end() const                 { return DataEnd; }

    // Utilities
    inline int  index_from_ptr(const _T* it) const   { IM_ASSERT(it >= Data && it < DataEnd); const ptrdiff_t off = it - Data; return (int)off; }
};
...
struct ImSpanAllocator
{
    ...
    template<typename _T>
    inline void  GetSpan(int n, ImSpan<_T>* span)    { span->set((_T*)GetSpanPtrBegin(n), (_T*)GetSpanPtrEnd(n)); }
};
...
{
    ImVector<_T>     Buf;        // Contiguous data
    ...
    ~ImPool()   { Clear(); }
    _T*          GetByKey(ImGuiID key)               { int idx = Map.GetInt(key, -1); return (idx != -1) ? &Buf[idx] : NULL; }
    _T*          GetByIndex(ImPoolIdx n)             { return &Buf[n]; }
    ImPoolIdx   GetIndex(const _T* p) const          { IM_ASSERT(p >= Buf.Data && p < Buf.Data + Buf.Size); return (ImPoolIdx)(p - Buf.Data); }
    _T*          GetOrAddByKey(ImGuiID key)          { int* p_idx = Map.GetIntRef(key, -1); if (*p_idx != -1) return &Buf[*p_idx]; *p_idx = FreeIdx; return Add(); }
    bool        Contains(const _T* p) const          { return (p >= Buf.Data && p < Buf.Data + Buf.Size); }
    void        Clear()                             { for (int n = 0; n < Map.Data.Size; n++) { int idx = Map.Data[n].val_i; if (idx != -1) Buf[idx].~T(); } Map.Clear(); Buf.clear(); FreeIdx = AliveCount = 0; }
    _T*          Add()                               { int idx = FreeIdx; if (idx == Buf.Size) { Buf.resize(Buf.Size + 1); FreeIdx++; } else { FreeIdx = *(int*)&Buf[idx]; } IM_PLACEMENT_NEW(&Buf[idx]) _T(); AliveCount++; return &Buf[idx]; }
    void        Remove(ImGuiID key, const _T* p)     { Remove(key, GetIndex(p)); }
    ...
    _T*          TryGetMapData(ImPoolIdx n)          { int idx = Map.Data[n].val_i; if (idx == -1) return NULL; return GetByIndex(idx); }
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
struct IMGUI_API ImChunkStream
{
    ...
    _T*      alloc_chunk(size_t sz)      { size_t HDR_SZ = 4; sz = IM_MEMALIGN(HDR_SZ + sz, 4u); int off = Buf.Size; Buf.resize(off + (int)sz); ((int*)(void*)(Buf.Data + off))[0] = (int)sz; return (_T*)(void*)(Buf.Data + off + (int)HDR_SZ); }
    _T*      begin()                     { size_t HDR_SZ = 4; if (!Buf.Data) return NULL; return (_T*)(void*)(Buf.Data + HDR_SZ); }
    _T*      next_chunk(_T* p)            { size_t HDR_SZ = 4; IM_ASSERT(p >= begin() && p < end()); p = (_T*)(void*)((char*)(void*)p + chunk_size(p)); if (p == (_T*)(void*)((char*)end() + HDR_SZ)) return (_T*)0; IM_ASSERT(p < end()); return p; }
    int     chunk_size(const _T* p)      { return ((const int*)p)[-1]; }
    _T*      end()                       { return (_T*)(void*)(Buf.Data + Buf.Size); }
    int     offset_from_ptr(const _T* p) { IM_ASSERT(p >= begin() && p < end()); const ptrdiff_t off = (const char*)p - Buf.Data; return (int)off; }
    _T*      ptr_from_offset(int off)    { IM_ASSERT(off >= 4 && off < Buf.Size); return (_T*)(void*)(Buf.Data + off); }
    void    swap(ImChunkStream<_T>& rhs) { rhs.Buf.swap(Buf); }
...
namespace ImGui
{
    ...
    template<typename _T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API float ScaleRatioFromValueT(ImGuiDataType data_type, _T v, _T v_min, _T v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_size);
    template<typename _T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API _T     ScaleValueFromRatioT(ImGuiDataType data_type, float t, _T v_min, _T v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_size);
    template<typename _T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API bool  DragBehaviorT(ImGuiDataType data_type, _T* v, float v_speed, _T v_min, _T v_max, const char* format, ImGuiSliderFlags flags);
    template<typename _T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API bool  SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, _T* v, _T v_min, _T v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb);
    template<typename _T, typename SIGNED_T>                     IMGUI_API _T     RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, _T v);
    template<typename _T>                                        IMGUI_API bool  CheckboxFlagsT(const char* label, _T* flags, _T flags_value);
