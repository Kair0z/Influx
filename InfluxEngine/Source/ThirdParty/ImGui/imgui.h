...
template<typename _T> void IM_DELETE(_T* p)   { if (p) { p->~T(); ImGui::MemFree(p); } }
...
struct ImVector
{
    ...
    inline ImVector(const ImVector<_T>& src)                 { Size = Capacity = 0; Data = NULL; operator=(src); }
    inline ImVector<_T>& operator=(const ImVector<_T>& src)   { clear(); resize(src.Size); memcpy(Data, src.Data, (size_t)Size * sizeof(_T)); return *this; }
    ...
    inline int          size_in_bytes() const               { return Size * (int)sizeof(_T); }
    inline int          max_size() const                    { return 0x7FFFFFFF / (int)sizeof(_T); }
    inline int          capacity() const                    { return Capacity; }
    inline _T&           operator[](int i)                   { IM_ASSERT(i >= 0 && i < Size); return Data[i]; }
    inline const _T&     operator[](int i) const             { IM_ASSERT(i >= 0 && i < Size); return Data[i]; }

    inline _T*           begin()                             { return Data; }
    inline const _T*     begin() const                       { return Data; }
    inline _T*           end()                               { return Data + Size; }
    inline const _T*     end() const                         { return Data + Size; }
    inline _T&           front()                             { IM_ASSERT(Size > 0); return Data[0]; }
    inline const _T&     front() const                       { IM_ASSERT(Size > 0); return Data[0]; }
    inline _T&           back()                              { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    inline const _T&     back() const                        { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    inline void         swap(ImVector<_T>& rhs)              { int rhs_size = rhs.Size; rhs.Size = Size; Size = rhs_size; int rhs_cap = rhs.Capacity; rhs.Capacity = Capacity; Capacity = rhs_cap; _T* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data; }

    inline int          _grow_capacity(int sz) const        { int new_capacity = Capacity ? (Capacity + Capacity / 2) : 8; return new_capacity > sz ? new_capacity : sz; }
    inline void         resize(int new_size)                { if (new_size > Capacity) reserve(_grow_capacity(new_size)); Size = new_size; }
    inline void         resize(int new_size, const _T& v)    { if (new_size > Capacity) reserve(_grow_capacity(new_size)); if (new_size > Size) for (int n = Size; n < new_size; n++) memcpy(&Data[n], &v, sizeof(v)); Size = new_size; }
    inline void         shrink(int new_size)                { IM_ASSERT(new_size <= Size); Size = new_size; } // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; _T* new_data = (_T*)IM_ALLOC((size_t)new_capacity * sizeof(_T)); if (Data) { memcpy(new_data, Data, (size_t)Size * sizeof(_T)); IM_FREE(Data); } Data = new_data; Capacity = new_capacity; }

    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
    inline void         push_back(const _T& v)               { if (Size == Capacity) reserve(_grow_capacity(Size + 1)); memcpy(&Data[Size], &v, sizeof(v)); Size++; }
    inline void         pop_back()                          { IM_ASSERT(Size > 0); Size--; }
    inline void         push_front(const _T& v)              { if (Size == 0) push_back(v); else insert(Data, v); }
    inline _T*           erase(const _T* it)                  { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(_T)); Size--; return Data + off; }
    inline _T*           erase(const _T* it, const _T* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last > it && it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, ((size_t)Size - (size_t)off - count) * sizeof(_T)); Size -= (int)count; return Data + off; }
    inline _T*           erase_unsorted(const _T* it)         { IM_ASSERT(it >= Data && it < Data + Size);  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) memcpy(Data + off, Data + Size - 1, sizeof(_T)); Size--; return Data + off; }
    inline _T*           insert(const _T* it, const _T& v)     { IM_ASSERT(it >= Data && it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(_T)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
    inline bool         contains(const _T& v) const          { const _T* data = Data;  const _T* data_end = Data + Size; while (data < data_end) if (*data++ == v) return true; return false; }
    inline _T*           find(const _T& v)                    { _T* data = Data;  const _T* data_end = Data + Size; while (data < data_end) if (*data == v) break; else ++data; return data; }
    inline const _T*     find(const _T& v) const              { const _T* data = Data;  const _T* data_end = Data + Size; while (data < data_end) if (*data == v) break; else ++data; return data; }
    inline bool         find_erase(const _T& v)              { const _T* it = find(v); if (it < Data + Size) { erase(it); return true; } return false; }
    inline bool         find_erase_unsorted(const _T& v)     { const _T* it = find(v); if (it < Data + Size) { erase_unsorted(it); return true; } return false; }
    inline int          index_from_ptr(const _T* it) const   { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; return (int)off; }
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
