...
namespace influx::graphics
{
	...
	ptr<graph_pipeline> dx12_device::create_workgraph_pipeline(rootsignature* rootsig, const graph_pipeline_desc& desc)
	{
		...
		workgraph_desc->SetProgramName(program_name.c_wstr());
		...
		const auto program_id = stateobj_props->GetProgramIdentifier(program_name.c_wstr());
		...
		const uint32 graph_index = workgraph_props->GetWorkGraphIndex(program_name.c_wstr());
