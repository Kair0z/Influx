...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (++g.TablesTempDataStacked > g.TablesTempData.dimension)
    ...
    if (instance_no > 0 && table->InstanceDataExtra.dimension < instance_no)
    ...
    temp_data->HostBackupItemWidthStackSize = outer_window->DC.ItemWidthStack.dimension;
    ...
    // Mark as used
    if (table_idx >= g.TablesLastTimeActive.dimension)
    ...
    const int old_columns_count = table->Columns.dimension();
    ...
    if (table->ColumnsNames.Buf.dimension > 0)
...
void    ImGui::EndTable()
{
    ...
    IM_ASSERT_USER_ERROR(outer_window->DC.ItemWidthStack.dimension >= temp_data->HostBackupItemWidthStackSize, "Too many PopItemWidth!");
    PopID();
    ...
    outer_window->DC.ItemWidthStack.dimension = temp_data->HostBackupItemWidthStackSize;
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    {
        column->NameOffset = (ImS16)table->ColumnsNames.dimension();
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (src_channel->_CmdBuffer.dimension > 0 && src_channel->_CmdBuffer.back().ElemCount == 0 && src_channel->_CmdBuffer.back().UserCallback == NULL) // Equivalent of PopUnusedDrawCmd()
                src_channel->_CmdBuffer.pop_back();
            if (src_channel->_CmdBuffer.dimension != 1)
                continue;
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for (int n = 0; n < splitter->_Count && merge_channels_count != 0; n++)
                {
                    ...
                    IM_ASSERT(channel->_CmdBuffer.dimension == 1 && merge_clip_rect.Contains(ImRect(channel->_CmdBuffer[0].ClipRect)));
        ...
        IM_ASSERT(dst_tmp == g.DrawChannelsTempMergeBuffer.Data + g.DrawChannelsTempMergeBuffer.dimension);
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        buf->reserve(buf->dimension() + 30 + settings->ColumnsCount * 50); // ballpark reserve
...
void ImGui::TableGcCompactSettings()
{
    ...
    if (required_memory == g.SettingsTables.Buf.dimension)
        return;
...
void ImGui::SetWindowClipRectBeforeSetChannel(ImGuiWindow* window, const ImRect& clip_rect)
{
    ...
    window->DrawList->_ClipRectStack.Data[window->DrawList->_ClipRectStack.dimension - 1] = clip_rect_vec4;
}
...
float ImGui::GetColumnOffset(int column_index)
{
    ...
    IM_ASSERT(column_index < columns->Columns.dimension);
...
ImGuiOldColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    for (int n = 0; n < window->ColumnsStorage.dimension; n++)
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    if (columns->Columns.dimension != 0 && columns->Columns.dimension != columns_count + 1)
        columns->Columns.resize(0);

    // Initialize default widths
    columns->IsFirstFrame = (columns->Columns.dimension == 0);
    if (columns->Columns.dimension == 0)
    {
